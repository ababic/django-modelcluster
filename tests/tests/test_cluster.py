...
class ClusterTest(TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.the_yellow_submarine = Restaurant.objects.create(
            name="The Yellow Submarine", proprietor=cls.marco_pierre_white
        )
    ...
    def test_can_create_cluster(self):
        ...
        self.assertEqual(
            "Paul McCartney", beatles.members.filter(name="Paul McCartney")[0].name
        )
        self.assertEqual(
            "Paul McCartney",
            beatles.members.filter(name__exact="Paul McCartney")[0].name,
        )
        self.assertEqual(
            "Paul McCartney",
            beatles.members.filter(name__iexact="paul mccartNEY")[0].name,
        )
        ...
        self.assertEqual(
            "John Lennon", beatles.members.filter(name__lt="Paul McCartney")[0].name
        )
        ...
        self.assertEqual(
            "Paul McCartney", beatles.members.filter(name__gte="M")[0].name
        )
        self.assertEqual(1, beatles.members.filter(name__gte="Paul McCartney").count())
        self.assertEqual(
            "Paul McCartney", beatles.members.filter(name__gte="Paul McCartney")[0].name
        )
        ...
        self.assertEqual(
            "Paul McCartney", beatles.members.filter(name__contains="Cart")[0].name
        )
        self.assertEqual(1, beatles.members.filter(name__icontains="carT").count())
        self.assertEqual(
            "Paul McCartney", beatles.members.filter(name__icontains="carT")[0].name
        )

        self.assertEqual(
            1,
            beatles.members.filter(
                name__in=["Paul McCartney", "Linda McCartney"]
            ).count(),
        )
        self.assertEqual(
            "Paul McCartney",
            beatles.members.filter(name__in=["Paul McCartney", "Linda McCartney"])[
                0
            ].name,
        )
        ...
        self.assertEqual(
            "Paul McCartney", beatles.members.filter(name__startswith="Paul")[0].name
        )
        self.assertEqual(1, beatles.members.filter(name__istartswith="pauL").count())
        self.assertEqual(
            "Paul McCartney", beatles.members.filter(name__istartswith="pauL")[0].name
        )
        self.assertEqual(1, beatles.members.filter(name__endswith="ney").count())
        self.assertEqual(
            "Paul McCartney", beatles.members.filter(name__endswith="ney")[0].name
        )
        self.assertEqual(1, beatles.members.filter(name__iendswith="Ney").count())
        self.assertEqual(
            "Paul McCartney", beatles.members.filter(name__iendswith="Ney")[0].name
        )

        self.assertEqual(
            "Paul McCartney", beatles.members.get(name="Paul McCartney").name
        )
        self.assertEqual(
            "Paul McCartney", beatles.members.get(name__exact="Paul McCartney").name
        )
        self.assertEqual(
            "Paul McCartney", beatles.members.get(name__iexact="paul mccartNEY").name
        )
        self.assertEqual(
            "John Lennon", beatles.members.get(name__lt="Paul McCartney").name
        )
        ...
        self.assertEqual(
            "Paul McCartney", beatles.members.get(name__gte="Paul McCartney").name
        )
        self.assertEqual(
            "Paul McCartney", beatles.members.get(name__contains="Cart").name
        )
        self.assertEqual(
            "Paul McCartney", beatles.members.get(name__icontains="carT").name
        )
        self.assertEqual(
            "Paul McCartney",
            beatles.members.get(name__in=["Paul McCartney", "Linda McCartney"]).name,
        )
        self.assertEqual(
            "Paul McCartney", beatles.members.get(name__startswith="Paul").name
        )
        self.assertEqual(
            "Paul McCartney", beatles.members.get(name__istartswith="pauL").name
        )
        self.assertEqual(
            "Paul McCartney", beatles.members.get(name__endswith="ney").name
        )
        self.assertEqual(
            "Paul McCartney", beatles.members.get(name__iendswith="Ney").name
        )
        ...
        self.assertRaises(
            BandMember.DoesNotExist, lambda: beatles.members.get(name="Reginald Dwight")
        )
        self.assertRaises(
            BandMember.MultipleObjectsReturned, lambda: beatles.members.get()
        )
        ...
        self.assertTrue(
            beatles.members.extra(
                where=["tests_bandmember.name='John Lennon'"]
            ).exists()
        )
    ...
    def test_values_list(self):
        beatles = Band(
            ...
            members=[
                BandMember(
                    name="John Lennon", favourite_restaurant=self.strawberry_fields
                ),
                BandMember(
                    name="Paul McCartney",
                    favourite_restaurant=self.the_yellow_submarine,
                ),
        ...
        self.assertEqual(
            [NAME_ONLY_TUPLE],
            list(beatles.members.filter(name="Paul McCartney").values_list("name")),
        )

        # 'fields' can span relationships using '__'
        members = beatles.members.all().values_list(
            "name", "favourite_restaurant__proprietor__name"
        )
        self.assertEqual(
            list(members),
            [
        ...
        self.assertEqual(
            NAME_ONLY_TUPLE,
            beatles.members.filter(name="Paul McCartney").values_list("name").get(),
        )

        # first() should return a tuple if used after values_list()
        self.assertEqual(
            NAME_ONLY_TUPLE,
            beatles.members.filter(name="Paul McCartney").values_list("name").first(),
        )

        # last() should return a tuple if used after values_list()
        self.assertEqual(
            NAME_ONLY_TUPLE,
            beatles.members.filter(name="Paul McCartney").values_list("name").last(),
        )
    ...
    def test_values(self):
            ...
            [
                {
                    ...
                    "band": None,
                    "name": "Paul McCartney",
                    "favourite_restaurant": self.the_yellow_submarine.id,
                }
            ],
        ...
        self.assertEqual(
            [NAME_ONLY_DICT],
            list(beatles.members.filter(name="Paul McCartney").values("name")),
        )

        # 'fields' can span relationships using '__'
        members = beatles.members.all().values(
            "name", "favourite_restaurant__proprietor__name"
        )
        self.assertEqual(
            list(members),
            [
                ...
                {
                    "name": "Paul McCartney",
                    "favourite_restaurant__proprietor__name": "Marco Pierre White",
                },
                {
                    "name": "George Harrison",
                    "favourite_restaurant__proprietor__name": None,
                },
        ...
        self.assertEqual(
            NAME_ONLY_DICT,
            beatles.members.filter(name="Paul McCartney").values("name").get(),
        )

        # first() should return a dict if used after values_list()
        self.assertEqual(
            NAME_ONLY_DICT,
            beatles.members.filter(name="Paul McCartney").values("name").first(),
        )

        # last() should return a dict if used after values_list()
        self.assertEqual(
            NAME_ONLY_DICT,
            beatles.members.filter(name="Paul McCartney").values("name").last(),
        )
    ...
    def test_queryset_filtering_on_models_with_inheritance(self):
        ...
        ringo = BandMember(
            name="Ringo Starr",
            favourite_restaurant=Restaurant.objects.get(name="The Yellow Submarine"),
        )
        ...
        self.assertEqual(
            list(
                beatles.members.filter(
                    favourite_restaurant=Place.objects.get(name="Strawberry Fields")
                )
            ),
            [john],
        )
        ...
        self.assertEqual(
            list(
                beatles.members.filter(favourite_restaurant=self.the_yellow_submarine)
            ),
            [ringo],
        )
    ...
    def test_queryset_exclude_filtering(self):
        ...
        self.assertEqual(
            "John Lennon", beatles.members.exclude(name="Paul McCartney").first().name
        )

        self.assertEqual(
            1, beatles.members.exclude(name__exact="Paul McCartney").count()
        )
        self.assertEqual(
            "John Lennon",
            beatles.members.exclude(name__exact="Paul McCartney").first().name,
        )
        self.assertEqual(
            1, beatles.members.exclude(name__iexact="paul mccartNEY").count()
        )
        self.assertEqual(
            "John Lennon",
            beatles.members.exclude(name__iexact="paul mccartNEY").first().name,
        )

        self.assertEqual(1, beatles.members.exclude(name__lt="M").count())
        self.assertEqual(
            "Paul McCartney", beatles.members.exclude(name__lt="M").first().name
        )
        self.assertEqual(1, beatles.members.exclude(name__lt="Paul McCartney").count())
        self.assertEqual(
            "Paul McCartney",
            beatles.members.exclude(name__lt="Paul McCartney").first().name,
        )

        self.assertEqual(1, beatles.members.exclude(name__lte="John Lennon").count())
        self.assertEqual(
            "Paul McCartney",
            beatles.members.exclude(name__lte="John Lennon").first().name,
        )

        self.assertEqual(1, beatles.members.exclude(name__gt="M").count())
        self.assertEqual(
            "John Lennon", beatles.members.exclude(name__gt="M").first().name
        )
        self.assertEqual(1, beatles.members.exclude(name__gte="Paul McCartney").count())
        self.assertEqual(
            "John Lennon",
            beatles.members.exclude(name__gte="Paul McCartney").first().name,
        )

        self.assertEqual(1, beatles.members.exclude(name__contains="Cart").count())
        self.assertEqual(
            "John Lennon", beatles.members.exclude(name__contains="Cart").first().name
        )
        self.assertEqual(1, beatles.members.exclude(name__icontains="carT").count())
        self.assertEqual(
            "John Lennon", beatles.members.exclude(name__icontains="carT").first().name
        )

        self.assertEqual(
            1,
            beatles.members.exclude(
                name__in=["Paul McCartney", "Linda McCartney"]
            ).count(),
        )
        self.assertEqual(
            "John Lennon",
            beatles.members.exclude(name__in=["Paul McCartney", "Linda McCartney"])[
                0
            ].name,
        )
        ...
        self.assertEqual(
            "John Lennon", beatles.members.exclude(name__startswith="Paul").first().name
        )
        self.assertEqual(1, beatles.members.exclude(name__istartswith="pauL").count())
        self.assertEqual(
            "John Lennon",
            beatles.members.exclude(name__istartswith="pauL").first().name,
        )
        self.assertEqual(1, beatles.members.exclude(name__endswith="ney").count())
        self.assertEqual(
            "John Lennon", beatles.members.exclude(name__endswith="ney").first().name
        )
        self.assertEqual(1, beatles.members.exclude(name__iendswith="Ney").count())
        self.assertEqual(
            "John Lennon", beatles.members.exclude(name__iendswith="Ney").first().name
        )
    ...
    def test_date_filters(self):
        ...
        logs = FakeQuerySet(
            ...
            [
                ...
                Log(
                    time=datetime.datetime(1980, 2, 2, 2, 2, 2), data="one person died"
                ),
        ...
        self.assertEqual(
            logs.get(time__date=datetime.date(1980, 2, 2)).data, "one person died"
        )

        self.assertEqual(tmbg.albums.get(release_date__year="1994").name, "John Henry")
        self.assertEqual(logs.get(time__year=1980).data, "one person died")

        self.assertEqual(
            tmbg.albums.get(release_date__iso_year="1994").name, "John Henry"
        )
        self.assertEqual(logs.get(time__iso_year=1980).data, "one person died")

        self.assertEqual(tmbg.albums.get(release_date__month=7).name, "John Henry")
        self.assertEqual(logs.get(time__month="2").data, "one person died")

        self.assertEqual(tmbg.albums.get(release_date__day="21").name, "John Henry")
        self.assertEqual(logs.get(time__day=2).data, "one person died")

        self.assertEqual(tmbg.albums.get(release_date__week=29).name, "John Henry")
        self.assertEqual(logs.get(time__week="5").data, "one person died")

        self.assertEqual(tmbg.albums.get(release_date__week_day=5).name, "John Henry")
        self.assertEqual(logs.get(time__week_day=7).data, "one person died")

        self.assertEqual(
            tmbg.albums.get(release_date__iso_week_day=4).name, "John Henry"
        )
        self.assertEqual(logs.get(time__iso_week_day=6).data, "one person died")

        self.assertEqual(tmbg.albums.get(release_date__quarter=3).name, "John Henry")
        self.assertEqual(logs.get(time__quarter=1).data, "one person died")

        self.assertEqual(
            logs.get(time__time=datetime.time(2, 2, 2)).data, "one person died"
        )
    ...
    def test_datetime_derivatives_with_values(self):
        ...
        self.assertEqual(
            ...
            [
                {
                    "time__date": datetime.date(1979, 7, 1),
                    "time__time": datetime.time(1, 1, 1),
                },
                {
                    "time__date": datetime.date(1980, 2, 2),
                    "time__time": datetime.time(2, 2, 2),
                },
            ...
            [
                {
                    "time__week": 26,
                    "time__week_day": 1,
                    "time__iso_week_day": 7,
                    "time__quarter": 3,
                },
                {
                    "time__week": 5,
                    "time__week_day": 7,
                    "time__iso_week_day": 6,
                    "time__quarter": 1,
                },
                {
                    "time__week": None,
                    "time__week_day": None,
                    "time__iso_week_day": None,
                    "time__quarter": None,
                },
            ],
        )
    ...
    def test_queryset_filtering_accross_foreignkeys(self):
        band = Band(
            ...
            members=[
                ...
                BandMember(
                    name="Ringo Starr", favourite_restaurant=self.the_yellow_submarine
                ),
            ],
        )
        ...
        self.assertEqual(
            tuple(
                band.members.filter(favourite_restaurant__name="The Yellow Submarine")
            ),
        ...
        self.assertEqual(
            tuple(
                band.members.filter(
                    favourite_restaurant__proprietor__name__iexact="gORDON rAMSAY"
                )
            ),
            (band.members.get(name="John Lennon"),),
        )
        self.assertEqual(
            tuple(
                band.members.filter(
                    favourite_restaurant__proprietor__name__iexact="mARCO pIERRE wHITE"
                )
            ),
            (band.members.get(name="Ringo Starr"),),
        )
        ...
        self.assertEqual(
            tuple(
                band.members.filter(favourite_restaurant__proprietor=self.gordon_ramsay)
            ),
            (band.members.get(name="John Lennon"),),
        )
        self.assertEqual(
            tuple(
                band.members.filter(
                    favourite_restaurant__proprietor=self.marco_pierre_white
                )
            ),
            (band.members.get(name="Ringo Starr"),),
        )
    ...
    def test_filtering_via_models_Q_objects(self):
        ...
        self.assertEqual(
            tuple(
                band.members.filter(
                    Q(name="John Lennon")
                    & Q(favourite_restaurant__name="Strawberry Fields")
                )
            ),
            (band.members.get(name="John Lennon"),),
        )
    ...
    def test_filtering_via_manytomany_raises_exception(self):
        ...
        underfloor_heating = Feature.objects.create(
            name="Underfloor heading", desirability=10
        )
        ...
        modern_living_room = Room.objects.create(
            name="Modern living room",
            features=[bay_window, underfloor_heating, log_burner],
        )
        classic_living_room = Room.objects.create(
            name="Classic living room", features=[bay_window, open_fire]
        )

        modern_house = House.objects.create(
            name="Modern house",
            address="1 Yellow Brick Road",
            main_room=modern_living_room,
        )
        classic_house = House.objects.create(
            name="Classic house",
            address="3 Yellow Brick Road",
            main_room=classic_living_room,
        )

        tenant = Person(name="Alex", houses=[modern_house, classic_house])
    ...
    def test_prefetch_related(self):
        ...
        with self.assertNumQueries(2):
            lists = [
                ...
                for band in Band.objects.prefetch_related("members")
            ]
        ...
        with self.assertNumQueries(2):
            lists = [
                ...
                for band in Band.objects.prefetch_related(
                    Prefetch(
                        "members",
                        queryset=BandMember.objects.filter(name__startswith="Paul"),
                    )
                )
            ]
        normal_lists = [
            list(band.members.filter(name__startswith="Paul"))
    ...
    def test_order_by_with_multiple_fields(self):
        ...
        albums = [
            album.name for album in beatles.albums.order_by("sort_order", "-name")
        ]
    ...
    def test_distinct_with_no_fields(self):
        ...
        albums = [
            album.name for album in beatles.albums.order_by("sort_order").distinct()
        ]
    ...
    def test_distinct_with_fields(self):
        ...
        albums = [
            album.name
            for album in beatles.albums.order_by("sort_order").distinct("sort_order")
        ]
        self.assertEqual(["Please Please Me", "With The Beatles"], albums)

        albums = [
            album.name
            for album in beatles.albums.order_by("sort_order").distinct("name")
        ]
        self.assertEqual(["Please Please Me", "With The Beatles", "Abbey Road"], albums)
    ...
    def test_parental_key_checks_clusterable_model(self):
        ...
        self.assertEqual(
            error.msg, "ParentalKey must point to a subclass of ClusterableModel."
        )
        self.assertEqual(
            error.hint,
            "Change tests.BandMember into a ClusterableModel or use a ForeignKey instead.",
        )
    ...
    def test_parental_key_checks_related_name_is_not_plus(self):
        ...
        self.assertEqual(
            error.msg, "related_name='+' is not allowed on ParentalKey fields"
        )
    ...
    def test_parental_key_checks_target_is_resolved_as_class(self):
        ...
        self.assertEqual(
            error.msg,
            "Field defines a relation with model 'Banana', which is either not installed, or is abstract.",
        )
...
class ParentalM2MTest(TestCase):
    ...
    def test_constructor(self):
        ...
        self.assertEqual(
            ["Author 1"], [author.name for author in article2.authors.order_by("name")]
        )
...
class ParentalManyToManyPrefetchTests(TestCase):
    ...
    def get_author_names(self, articles):
        return [author.name for article in articles for author in article.authors.all()]
...
class PrefetchRelatedTest(TestCase):
    def test_fakequeryset_prefetch_related(self):
        ...
        house1 = House.objects.create(
            name="House 1", address="123 Main St", owner=person1
        )
    ...
    def test_prefetch_related_with_lookup(self):
        ...
        menu_item1 = MenuItem.objects.create(
            restaurant=restaurant1, dish=dish1, recommended_wine=wine1, price=1
        )
        menu_item2 = MenuItem.objects.create(
            restaurant=restaurant2, dish=dish2, recommended_wine=wine2, price=10
        )
        ...
        query = Restaurant.objects.all().prefetch_related(
            Prefetch(
                ...
                queryset=MenuItem.objects.only(
                    "price", "recommended_wine"
                ).select_related("recommended_wine"),
            )
        )
    ...
    def test_m2m_prefetch_related_with_lookup(self):
        ...
        house = House.objects.create(
            name="House 1", address="123 Main St", owner=person1, main_room=room
        )
        person1.houses = [house]
        person1.save()

        query = Person.objects.order_by("name").prefetch_related(
